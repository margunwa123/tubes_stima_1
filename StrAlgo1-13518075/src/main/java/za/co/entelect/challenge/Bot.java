    ...
    public String run() {
        ...
        if(command.equals("")) {
            for(int i = 0 ; i < gameState.gameDetails.mapHeight ; i++) {
                int enemyAttackOnRow = enemyAttackerEachRows.get(i);
                int myDefenseOnRow   = myDefenseEachRows.get(i);
                if(enemyAttackOnRow >= 2 && myDefenseOnRow == 0 && canAffordBuilding(BuildingType.DEFENSE)) {
        ...
        if(command.equals("")) {
            if(canAffordBuilding(BuildingType.ATTACK)) {
                ...
                for(int i = 0; i < gameState.gameDetails.mapHeight; i++) {
                    ...
                    int myDefenseOnRow    = myDefenseEachRows.get(i);
                    enemyRowStrength.put(i,((enemyAttackOnRow * 5) + (enemyDefenseOnRow * 20) - (myAttackerOnRow * 5) - (myDefenseOnRow * 20)));
                }
                ...
                for(int i = 0 ; i < gameState.gameDetails.mapHeight ; i++) {
                    ...
                    sortedRowStrengthAsc.add(rowIndex);
